...
if(!empty($_POST["mpass_pass"])) {
  ...
  function getIp() {
    ...
    if ((md5($_POST["mpass_pass"]) == $cmp_pass) && $token_check == true) {
		...
		$_SESSION["mpass_pass"] = crypt(md5($_POST["mpass_pass"]));
		setcookie ("mpass_pass", crypt(md5($_POST["mpass_pass"])), time()+3600*24*7,'/');
...
if (($max_session_time > 0 
	...
	|| empty($_SESSION["mpass_pass"]) 
	|| !crypt($cmp_pass,$_SESSION["mpass_pass"] == $_SESSION["mpass_pass"])) {

	sleep(1);
	
	if (!(crypt($cmp_pass,$_SESSION["mpass_pass"] == $_SESSION["mpass_pass"]))) {	
